...
{
    internal class BlendShapeWeightSave
    {
        ...
        public float GetOriginalWeight(SkinnedMeshRenderer renderer, string name)
        {
            ...
            if (!originalValues[renderer].values.TryGetValue(name, out float weight)) return 0f;
            return weight;
        }
        ...
        public float GetPrefabWeight(SkinnedMeshRenderer renderer, string name)
        {
            ...
            if (!prefabValues[renderer].values.TryGetValue(name, out float weight)) return 0f;
            return weight;
        }
    }
}
